...
public class QueryWriter extends Task<Void> {
    private final Label label;
    private final Timeline animation;

    private final ArrayDeque<String> states = new ArrayDeque<>();

    public QueryWriter(Label label) {
        this.label = label;
    ...
    public KeyFrame createKeyFrame(String message) {
        return new KeyFrame(Duration.millis(1001), new KeyValue(label.textProperty(), message));
    }
